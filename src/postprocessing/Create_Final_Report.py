...
def reportCreation():
    ...
    save_combined = {'loCo': {'Entire': None, 'Masked': None}, '5': {'Entire': None, 'Masked': None}} if modality_set[modality.lower()] == 1 else {'loCo': {'serial': None, 'parallel': None}, '5': {'serial': None, 'parallel': None}, 'loCo6': {'serial': None, 'parallel': None}, 'loCo18': {'serial': None, 'parallel': None}}
            ...
            for Experiment_folder in Experiment_folders:
                ...
                for Model_folder in Models_folders:
                    ...
                    if modality_set[modality.lower()] == 2:
                        file_report = file_report + f'_AFC.xlsx'
                    else:
                ...
                df_resume_experiments.to_excel(
                    os.path.join(rep_version, f'Morbidity_models_report_{CV}_resume_exp_{save_name}_{version[1]}_{directory_id}.xlsx'))
    ...
    if modality_set[modality.lower()] == 0 or modality_set[modality.lower()] == 2:
        ...
        for CV in fold_cv:
            ...
            for Experiment_folder in Experiment_folders:
                ...
                if not os.path.exists(rep_version):
                    os.mkdir(rep_version)
                print(df_resume_experiments)
                df_resume_experiments.to_excel(
                    os.path.join(rep_version, f'Severity_models_report_{CV}_resume_exp_{save_name}_{version[1]}_{directory_id}.xlsx'))


                print('Experiment_Report: ', os.path.join(rep_version, f'Severity_models_report_{CV}_resume_exp_{save_name}_{version[1]}_{directory_id}.xlsx'))
