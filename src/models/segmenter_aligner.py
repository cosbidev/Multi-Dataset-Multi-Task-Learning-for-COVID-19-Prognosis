# This code is supposed to segment images using the net trained in tensorflow
import os
...
from .BSNet import build_xnet, STN, build_BScore, BilinearInterpolation, freeze_model, get_backbone
...
def AlignerSegmenter(
                      ...
                      seg_model_weights='./weights/segmentation_brixia-model.h5',
    ...
    Args:
        ...
        load_seg_model: (bool) wheter to load the segmentation_brixia model weighes. A proper path in `seg_model_weights` must be setted

        seg_model_weights: (str) path to a proper model file

        freeze_segmentation: (bool) Set segmentation_brixia layers weights as non-trainable. Useful for fine-tuning
        ...
        explict_self_attention: (bool) multiply the segmentation_brixia map
    ...
    if load_seg_model:
        ...
        print("Loading segmentation_brixia model")
        ...
        try:
            ...
            print(f"Loading a wrong weight checkpoint for segmentation_brixia model. {e}")

    # lock the segmentation_brixia network weights for fine-tuning
    ...
    align_model = BilinearInterpolation(input_shape[:2], name='alignmnet_segmentation')([input_data, locnet])


    if pretrain_aligment_net:

        align_model = tf.keras.Model(input_data, None)

    else:

        align_model = tf.keras.Model(backbone.input, align_model)
    return align_model, seg_model, locnet